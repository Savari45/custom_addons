...
class StockMove(models.Model):
    ...
    barcode = fields.Char(string="Barcode", help="Scan product barcode")

    @api.onchange('barcode')
    def _onchange_barcode_check_match(self):
        if self.barcode:
            # If product already selected in this move (line)
            if self.product_id and self.product_id.barcode != self.barcode:
                self.barcode = False
                raise UserError("Scanned barcode does not match the selected product.")
            # If product not set, set by barcode
