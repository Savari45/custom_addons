...
class GenerateXMLReport(models.TransientModel):
    ...
    def generate_header(self,root, company):
        ...
        try:
            number = address_parts[0] if address_parts else ""
        except Exception as e:
            number = ""
            _logger.warning(f"Error parsing address: {e}")
    ...
    def generate_invoice_lines(self, root, company):
        ...
        for invoice in invoices_ids:
            ...
            for line in invoice.invoice_line_ids:
                line_base = line.price_subtotal  # Base amount for the line
                for tax in line.tax_ids:
                    if tax.amount < 0:
                        # Calculate actual withholding amount (percentage)
                        if tax.amount_type == 'percent':
                            ...
                            tax_amount = 0  # You may need to loop child taxes here
            ...
            # Only create WithholdingTax tag if there's an amount
